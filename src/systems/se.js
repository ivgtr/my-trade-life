/**
 * se.js — DAY TRADER LIFE プロシージャルSE
 *
...
function getCtx() {
  if (!sePlayer._ctx) {
    sePlayer._ctx = new (window.AudioContext || window.webkitAudioContext)()
  }
 ...
 * 硬質で短いパーカッシブな電子音
 */
function playEntry(ctx, masterGain) {
 ...
 */
function playExit(ctx, masterGain) {
  const now = ctx.currentTime
  // 2音
 ...
 * 明るく短い上昇グリッサンド
 */
function playProfit(ctx, masterGain) {
  const now = ctx.currentTime
 ...
 * 鈍く短い下降グリッサンド
 */
function playLoss(ctx, masterGain) {
  const now   = ctx.currentTime
 ...
 * 不協和音 + ノイズバースト
 */
function playLosscut(ctx, masterGain) {
  const now = ctx.currentTime
 ...
 * 緊急感のある2音アラート（繰り返し）
 */
function playNews(ctx, masterGain) {
  const now    = ctx.currentTime
 ...
 * 短い上昇アルペジオ + チャイム
 */
function playLevelup(ctx, masterGain) {
  const now   = ctx.currentTime
 ...
 * 明るい上昇コード
 */
function playMilestone(ctx, masterGain) {
  const now   = ctx.currentTime
 ...
 * 派手なファンファーレ + バス
 */
function playMilestoneBig(ctx, masterGain) {
  const now = ctx.currentTime
 ...
 */
function playCalendarFlip(ctx, masterGain) {
  const now = ctx.currentTime
  const osc = ctx.createOscillator()
  const g   = ctx.createGain()
  osc.type  = 'sine'
