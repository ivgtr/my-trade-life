/**
 * bgm.js — DAY TRADER LIFE プロシージャルBGM
 *
...
function getCtx() {
  if (!bgmPlayer._ctx) {
    bgmPlayer._ctx = new (window.AudioContext || window.webkitAudioContext)()
  }
 ...
 */
function makeReverb(ctx, duration = 1.8, decay = 2.0) {
 ...
 * フェードイン/アウト用のスケジュール
 */
function fadeIn(gainNode, ctx, duration = 1.2) {
...
function fadeOut(gainNode, ctx, duration = 1.0, then) {
  const g = gainNode.gain
  g.cancelScheduledValues(ctx.currentTime)
 ...
 */
function buildTitle(ctx, masterGain) {
  const nodes  = []
 ...
 */
function buildTrading(ctx, masterGain) {
  const nodes = []
  const now   = ctx.currentTime
 ...
 * 音楽的特徴: シンプルなマリンバ風ループ
 */
function buildCalendar(ctx, masterGain) {
  const nodes   = []
 ...
 */
function buildReport(ctx, masterGain) {
  const nodes  = []
 ...
 */
function buildGameover(ctx, masterGain) {
  const nodes = []
  const now   = ctx.currentTime
...
export const bgmPlayer = {
  ...
  _stopNodes() {
    ...
    // タイマー停止
    ;(nodes._timers || []).forEach(t => clearInterval(t))
    // AudioNodeを停止
    nodes.forEach(node => {
      if (!node) return
      try {
